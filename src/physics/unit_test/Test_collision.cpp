...
// TEST(Collision, rigidBodyCollision)
// {
...
TEST(Collision, boundaryWallCollision)
{
    ...
    constexpr ConvexPolygon<4> domain{makePolygon<4>(scalar_t(1))}; // with so few vertices, this is a square rotated 45 degs
    bdry_manager.setBoundary(domain);

    auto & tri_manager {TrianglesManager::getSingleton()};
    constexpr auto circle {makePolygon<36>(1)};
